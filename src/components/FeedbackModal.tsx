      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 bg-black/40" onClick={e => e.target === e.currentTarget && setOpen(false)}>
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-md p-6 animate-slide-up">
            <h3 className="font-semibold text-gray-800 dark:text-white text-lg mb-1">💡 {t('feedback.modalTitle')}</h3>
            <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">{t('feedback.modalSubtitle')}</p>
            {sent ? (
              <div className="text-center py-8">
                <div className="text-3xl mb-2">✅</div>
                <div className="text-green-600 dark:text-green-400 font-medium">{t('feedback.thankYou')}</div>
                <p className="text-xs text-gray-400 mt-2">{t('feedback.thankYouSub')}</p>
              </div>
            ) : (
              <>
                <textarea
